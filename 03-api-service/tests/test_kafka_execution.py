...
async def test_execute_pipeline_publishes_complete_command(monkeypatch, pipeline):
    ...
    monkeypatch.setattr(kafka_runs, "now_iso", lambda: "2026-07-15T10:00:00+00:00")
...
def test_worker_builds_ordered_completed_logs(monkeypatch, worker_module, pipeline):
    ...
    monkeypatch.setattr(
        worker_module, "now_iso", lambda: "2026-07-15T10:00:01+00:00"
    )
    command = {
        "run_id": "RUN_123",
        "fabric_tx": "TX_456",
    ...
    assert all(log["ts"] == "2026-07-15T10:00:01+00:00" for log in logs)
