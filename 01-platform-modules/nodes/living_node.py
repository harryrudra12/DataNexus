...
class NodeCapability:
    ...
    def __init__(self, cpu_cores: int, ram_mb: int | None = None, storage_mb: int = 0,
                 has_gpu: bool = False, has_tee: bool = False, network_mbps: float = 10.0,
                 battery_pct: Optional[float] = None, memory_mb: int | None = None):
        # Backward-compatible alias: older tests/docs used memory_mb.
        if ram_mb is None:
            ram_mb = memory_mb
        if ram_mb is None:
            raise TypeError("NodeCapability requires ram_mb or memory_mb")
        self.cpu_cores = cpu_cores
        self.ram_mb = ram_mb
        self.storage_mb = storage_mb
        self.has_gpu = has_gpu
        self.has_tee = has_tee
...
class DataNexusNode:
    ...
    def __init__(
        ...
        fabric_peers: List[str] | None = None,     # peer node addresses
        private_key:  str = "",
        ledger_peers: List[str] | None = None,
    ):
        self.node_id      = f"dn-node-{uuid.uuid4().hex[:8]}"
        self.node_type    = node_type
        self.capability   = capability
        self.region       = region
        self.fabric_peers = fabric_peers if fabric_peers is not None else (ledger_peers or [])
        ...
        self.heal_log     = []
        self.sigma_scores = {}
        self.peer_health  = {p: True for p in self.fabric_peers}
